...
while True:
    ...
    if len(lmList) != 0:
        ...
        xThumbFinger, yThumbFinger = lmList[4][1:]
        ...
        # if only thumb and index finger are up (to play/pause video)
        if fingers[1] == 1 and fingers[2] == 0 and fingers[0] == 1:
            # distThumbIndex is distance between thumb and index finger.
            distThumbIndex, img, lineInfo = detector.findDistance(4, 8, img)
            print(distThumbIndex)

            if distThumbIndex <= 28:
                cv2.circle(
        ...
        # if no fingers are up, capture screenshot
        if fingers[0]==0 and fingers[1]==0 and fingers[2]==0 and fingers[3]==0 and fingers[4]==0:
            pyautogui.screenshot("screnshot.png")
